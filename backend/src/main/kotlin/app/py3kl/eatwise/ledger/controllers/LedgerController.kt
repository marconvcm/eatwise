...
class LedgerController(
    ...
    ): Flux<LedgerEntry> = ledgerEntryService.getAllEntriesAsync(profile)

    @PostMapping("/ledger/entries")
    fun createEntry(
        @CurrentUser profile: UserProfile,
        @RequestBody entry: LedgerEntryRequest
    ): Mono<LedgerEntry> = ledgerEntryService.createEntryAsync(profile, entry)
    ...
    fun updateEntry(
        ...
        @RequestBody entry: LedgerEntryRequest
    ): Mono<LedgerEntry> = ledgerEntryService.updateEntryAsync(profile, id, entry)

    @DeleteMapping("/ledger/entries/{id}")
    fun deleteEntry(
        @CurrentUser profile: UserProfile,
        @PathVariable("id") id: UUID
    ): Mono<Boolean> = ledgerEntryService.deleteEntryAsync(profile, id)
}
